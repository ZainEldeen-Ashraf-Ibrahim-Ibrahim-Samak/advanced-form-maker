...
- [x] T029 [P] [US4] Add `branding: { siteName: { type: String, default: "SCCT DAMAGES", maxlength: 100 }, siteLogoUrl: { type: String, default: "", maxlength: 500 } }` subdocument to `SettingsConfigurationSchema` in `src/data/models/settings.model.ts` and `ISettingsConfiguration` interface
